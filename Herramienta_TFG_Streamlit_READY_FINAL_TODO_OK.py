...
# === GENERACIÓN DE FIGURAS PARA EL INFORME DE GRÁFICAS ===
import matplotlib.pyplot as plt

figuras = []

# === GRÁFICA 1: Distribución del riesgo de abandono
fig_riesgo = plt.figure()
plt.hist(df_sim["Prob_Abandono"], bins=10, color='skyblue', edgecolor='black')
plt.title("Distribución del riesgo de abandono")
plt.xlabel("Probabilidad de Abandono")
plt.ylabel("Número de empleados")
figuras.append(fig_riesgo)

# === GRÁFICA 2: Clima laboral medio por departamento
fig_clima = plt.figure()
df_sim.groupby("Departamento")["Clima_Laboral"].mean().plot(kind="bar", color="salmon")
plt.title("Clima laboral medio por departamento")
plt.ylabel("Puntuación media")
plt.ylim(0, 5)
figuras.append(fig_clima)

# === GRÁFICA 3: Simulación de políticas (si existe df_politicas)
try:
    fig_politicas = plt.figure()
    df_politicas.plot(x="Escenario", y="Riesgo_Promedio", kind="bar", legend=False, color="lightgreen")
    plt.title("Impacto de políticas en el riesgo de abandono")
    plt.ylabel("Riesgo promedio")
    figuras.append(fig_politicas)
except:
    st.warning("No se pudo generar la gráfica de políticas (df_politicas no definido)")

# === GRÁFICA 4: Clustering (si existen columnas PCA1 y PCA2)
try:
    fig_clusters = plt.figure()
    for cluster in df_sim['Cluster'].unique():
        subset = df_sim[df_sim['Cluster'] == cluster]
        plt.scatter(subset['PCA1'], subset['PCA2'], label=f'Cluster {cluster}')
    plt.title("Visualización de Clustering de Empleados (PCA)")
    plt.xlabel("PCA1")
    plt.ylabel("PCA2")
    plt.legend()
    figuras.append(fig_clusters)
except:
    st.warning("No se pudo generar la gráfica de clustering (PCA1 o Cluster no definidos)")
...
# ===== GENERAR ARCHIVO TXT CON INFORME =====
def generar_informe_txt(df, nombre_archivo="informe_final.txt"):
    with open(nombre_archivo, "w", encoding="utf-8") as f:
        f.write("Informe Detallado de Análisis de Empleados\n")
        f.write("========================================\n\n")
        for i, fila in df.iterrows():
            f.write(f"Empleado {i+1}\n")
            f.write(f"Departamento: {fila['Departamento']}\n")
            f.write(f"Edad: {int(fila['Edad'])} años\n")
            f.write(f"Antigüedad: {int(fila['Antigüedad'])} años\n")
            f.write(f"Desempeño: {fila['Desempeño']} / 5\n")
            f.write(f"Clima Laboral: {fila['Clima_Laboral']} / 5\n")
            f.write(f"Horas Extra: {fila['Horas_Extra']}h\n")
            f.write(f"Tipo de Contrato: {fila['Tipo_Contrato']}\n")
            f.write(f"Bajas Último Año: {fila['Bajas_Ultimo_Año']}\n")
            f.write(f"Promociones en 2 Años: {fila['Promociones_2_Años']}\n")
            f.write(f"Probabilidad de Abandono: {int(fila['Prob_Abandono']*100)}%\n")
            f.write(f"Recomendación: {fila['Recomendación']}\n")
            f.write("\n" + "-"*40 + "\n\n")
    return nombre_archivo

# ===== GENERAR PDF CON SÓLO GRÁFICAS =====
def generar_pdf_graficas(figuras, nombre_archivo="informe_graficas.pdf"):
    pdf = FPDF()
    for figura in figuras:
        pdf.add_page()
        figura_path = "temp_figura.png"
        figura.savefig(figura_path, bbox_inches='tight')
        pdf.image(figura_path, x=10, y=20, w=180)
...
if st.button("📄 Generar informe TXT"):
    nombre_txt = generar_informe_txt(df_sim)
    with open(nombre_txt, "rb") as f:
        st.download_button("⬇️ Descargar informe TXT", f, file_name=nombre_txt)

# Botón PDF con gráficas
if st.button("📊 Generar PDF con gráficas"):
    # Aquí debes pasar tus gráficas generadas previamente
    # Ejemplo: figuras = [fig1, fig2, fig3, fig4]
    nombre_pdf = generar_pdf_graficas(figuras)
    with open(nombre_pdf, "rb") as f:
        st.download_button("⬇️ Descargar PDF de gráficas", f, file_name=nombre_pdf)
